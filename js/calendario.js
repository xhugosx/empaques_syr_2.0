function cargarCalendarioLamina() {
    ...
    // Obtiene todos los pedidos de un año y tipo (1 sola vez)
    function obtenerProductosDelAño(añoReq, tipo, callback) {
        let cacheKey = añoReq + "_" + tipo;
        if (productCache[cacheKey]) {
            return callback(productCache[cacheKey]);
        }

        let link = myLink + "/php/lista_pedidos_lamina/select.php?type=" + tipo + "&anio=" + añoReq;
        servidor(link, function (respuesta) {
            ...
            productCache[cacheKey] = objetosJSON;
    ...
    function mostrarCalendario() {
        ...
        obtenerProductosDelAño(año, tipo, function (productos) {
            // Mapa fecha → productos (+2 días)
            const mapaFechas = {};
