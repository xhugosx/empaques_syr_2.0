document.addEventListener('init', function (event) {
    ...
    if (page.id === 'actividad') {
        ...
        // Obtiene todos los registros de un año (una sola vez)
        function obtenerProductosDelAño(añoReq, callback) {
            if (productCache[añoReq]) {
                return callback(productCache[añoReq]);
            }
            oCarga("Buscando dias...");
            let link = myLink + "/php/actividad/select.php?anio=" + añoReq;
        ...
        function mostrarCalendario() {
            ...
            obtenerProductosDelAño(año, function (actividad) {
                // Mapa fecha -> productos
                const mapaFechas = {};
                actividad.forEach(act => {
                    let fecha = act.fecha_hora.split(' ')[0];
                    let fechaEntrega = sumarDiasAFecha(fecha, 1);
                    mapaFechas[fechaEntrega] = mapaFechas[fechaEntrega] || [];
                    mapaFechas[fechaEntrega].push(act);
                });
        ...
        diasElemento.addEventListener('click', function (e) {
            ...
            fechaActividad = hoy;
            let link = myLink + "/php/actividad/select.php?fecha=" + hoy;
            oCarga("Buscando Actividad...");
            servidor(link, function (respuesta) {
